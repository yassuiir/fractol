...
/*   Updated: 2024/05/02 20:13:09 by yait-nas         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	handle_pixel(t_fractol *mlx_data, int i, int j)
{
	...
	index = 0;
	c.real = (scale(i, -2, 2, 800) * mlx_data->ratio) + mlx_data->shift_x;
	c.im = (scale(j, 2, -2, 800) * mlx_data->ratio) - mlx_data->shift_y;
	mandel_vs_julia(&z, &c, mlx_data);
	while (index < mlx_data->iterations)
	{
		z = sum_complex(square_complex(z, mlx_data->argv), c);
		if ((z.real * z.real) + (z.im * z.im) > 4)
		{
			color = scale(index, BLACK, WHITE, mlx_data->depends);
